...
class LoRA(nn.Module):
    def __init__(self, dim, rank=4, alpha=1.0):
        ...
        # LoRA 分解矩阵
        self.A = nn.Linear(dim, rank, bias=False)  # 输入到低秩空间
        self.B = nn.Linear(rank, dim, bias=False)  # 从低秩空间映射回高维空间
...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        if self.layer_id == 0:
            self.ln0 = nn.LayerNorm(args.n_embd)  # only used in block 0
        self.ln1 = nn.LayerNorm(args.n_embd)
        self.ln2 = nn.LayerNorm(args.n_embd)
        self.ln_t = nn.LayerNorm(args.n_embd)
        self.ln_v = nn.LayerNorm(args.n_embd)

        self.att = RWKV_Tmix_x070(args, layer_id)
        self.ffn = RWKV_CMix_x070(args, layer_id)  # Frozen FFN
        self.lora_ffn_t = LoRA(args.n_embd)
        self.lora_ffn_v = LoRA(args.n_embd)
    ...
    def forward(self, x, mask, v_first):
        ...
        ffn_t_out = self.lora_ffn_t(self.ln_t(x))
        ffn_v_out = self.lora_ffn_v(self.ln_v(x))
        # 调整 mask 的形状以匹配 x
        if mask.shape[1] < x.shape[1]:
            padding = x.shape[1] - mask.shape[1]
            pad_mask = torch.zeros((mask.shape[0], padding, 1), dtype=mask.dtype, device=mask.device)
            mask = torch.cat([mask, pad_mask], dim=1)
        elif mask.shape[1] > x.shape[1]:
            mask = mask[:, :x.shape[1], :]
        x = x + torch.where(mask, ffn_v_out, ffn_t_out)
...
class RWKV(pl.LightningModule):
    ...
    def forward(self, x, mask):
        ...
        v_first = torch.empty_like(x)  # 如果没有图像输入，保持原样
...
class VisualRWKV(pl.LightningModule):
    ...
    def load_rwkv_from_pretrained(self, path):
        ...
        for block in self.rwkv.blocks:
            if hasattr(block, 'lora_ffn_t'):
                # 使用 Xavier 初始化 LoRA 层的权重
                nn.init.xavier_uniform_(block.lora_ffn_t.A.weight)
                nn.init.xavier_uniform_(block.lora_ffn_t.B.weight)
            if hasattr(block, 'lora_ffn_v'):
                nn.init.xavier_uniform_(block.lora_ffn_v.A.weight)
                nn.init.xavier_uniform_(block.lora_ffn_v.B.weight)
        
        # 加载预训练模型的权重
        state_dict = torch.load(path, map_location="cpu", weights_only=True)
        self.rwkv.load_state_dict(state_dict, strict=False)  # strict=False 允许忽略缺失的参数

    @property
    def deepspeed_offload(self) -> bool:  # 检查是否启用了 DeepSpeed 的优化器或参数卸载功能
    ...
    def freeze_ffn_train_lora(self):
        for block in self.rwkv.blocks:
            # 冻结 FFN 层
            block.ffn.requires_grad_(False)
            # 确保 LoRA 层的参数可训练
            block.lora_ffn_t.A.requires_grad_(True)
            block.lora_ffn_t.B.requires_grad_(True)
            block.lora_ffn_v.A.requires_grad_(True)
            block.lora_ffn_v.B.requires_grad_(True)
    ...
    def forward(self, samples):
        x, targets, mask = self.preparing_embedding(samples)
        # unidirectional forward
        logits = self.rwkv(x, mask)
    ...
    def training_step_end(self, batch_parts):  # 处理批量数据
        if pl.__version__[0] != '2':
            all = self.all_gather(batch_parts)
            if self.trainer.is_global_zero:
                self.trainer.my_loss_all = all

    def adaptive_pooling(self, image_features):  # 将输入的图像特征调整到一个固定的空间维度，同时保留特征的语义信息
    ...
    def preparing_embedding(self, samples):
        if "images" not in samples:
            # 如果没有图像输入，则直接返回输入嵌入和标签，并生成全零 mask
            input_embeds = self.rwkv.emb(samples["input_ids"])
            mask = torch.zeros(
                input_embeds.shape[:2] + (1,),  # [B, L, 1]
                dtype=torch.bool,
                device=input_embeds.device,
            )
            return input_embeds, samples["labels"], mask

        ### 准备图像特征
        image_features = self.encode_images(samples["images"])  # [B_IMG, L_IMG, D_IMG]
        B_IMG, L_IMG, D_IMG = image_features.shape
        image_features = image_features.view(-1, D_IMG)

        ### 准备输入 token 的嵌入
        input_embeds = self.rwkv.emb(samples["input_ids"])  # [B, L, D]
        B, L, D = input_embeds.shape
        input_embeds = input_embeds.view(B * L, D)
        input_ids = samples["input_ids"].view(B * L)

        ### 匹配 IMAGE_TOKEN_INDEX 的位置
        selected = (input_ids == IMAGE_TOKEN_INDEX)  # [B * L]
        selected_sum = selected.sum()

        if selected_sum != B_IMG * L_IMG:
            # 如果图像特征数量和 IMAGE_TOKEN_INDEX 数量不匹配，截断图像特征
        ...
        mask.view(-1)[selected] = True  # 对 selected 的位置设置为 True
    ...
    def generate(self, input_ids, images, do_sample, temperature, top_p, max_new_tokens, stop_token_idx) -> list[int]:
        ...
        x, _ , mask = self.preparing_embedding(samples)
        # generate
        generated_tokens = []
        generated_token_logits = []
        generated_token_probs = []
        for i in range(max_new_tokens):
            logits = self.rwkv(x, mask)[:, -1, :]
            if do_sample:
