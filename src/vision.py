...
class SamDinoSigLIPViTBackbone(nn.Module):
    def __init__(self, vision_tower_path: dict, default_image_size: int = 448) -> None:
        ...
        self.dino_featurizer = timm.create_model(  # 使用 timm 库创建的一个 Vision Transformer 模型
        ...
        # 解决特定的兼容性问题，确保 DINO 和 SIGLIP featurizer 能够与 Fully Sharded Data Parallel (FSDP) 配合使用。FSDP 是 PyTorch 中的一种并行训练策略
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:  # 三个编码器的图像进行拼接
