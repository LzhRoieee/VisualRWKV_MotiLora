...
class Block(nn.Module): # 窗口注意力和残差传播块
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: torch.Tensor) -> torch.Tensor:
    ...
    Get relative positional embeddings according to the relative positions of
        query and key sizes.
        根据查询query和键key的相对位置,获取相对位置嵌入relative positional embeddings
    Args:
    ...
    if checkpoint is not None:
        ...
        for k in list(state_dict.keys()):
            if k.startswith('image_encoder.'):
                new_state_dict[k[len('image_encoder.'):]] = state_dict[k]
        msg = image_encoder.load_state_dict(new_state_dict, strict=False)
        print(f'Loaded sam image encoder from {checkpoint} with msg: {msg}')
    return image_encoder


if __name__ == '__main__':
    x = torch.zeros(2, 3, 1024, 1024)  # 这个张量占用的内存24GB
